...
class UserController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = User::withTrashed()
            ...
            ->whereRaw('? < (CASE role
                ...
                ELSE 99 END)', [UserPolicy::level($actor->role)]);
        ...
        // Load employee relationship for staff users
        $users = $query->paginate($perPage);
        $users->load(['employee.manager.user']);
    ...
    {
        $user = User::withTrashed()->with(['customer', 'employee.manager.user'])->findOrFail($id);
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'role'        => 'required|in:admin,manager,sales_rep,customer',
        ...
        try {
            ...
            if ($user->role === 'customer') {
                ...
                // Create employee record for staff members
                Employee::create([
                    'user_id' => $user->id,
                    'employee_id' => $request->employee_id,
            ...
            return response()->json([
                ...
                'data' => $user->load(['customer', 'employee.manager.user'])
            ], 201);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'company_name'        => 'nullable|string|max:255',
            'role'                => 'sometimes|in:admin,manager,sales_rep,customer',
        ...
        try {
            ...
            // Handle role change (customer <-> staff)
            if ($request->filled('role') && $request->role !== $oldRole) {
                if ($request->role === 'customer' && in_array($oldRole, ['admin', 'manager', 'sales_rep'])) {
                    // Changed from staff to customer - delete employee record
                    if ($user->employee) {
                ...
                } elseif (in_array($request->role, ['admin', 'manager', 'sales_rep']) && $oldRole === 'customer') {
                    // Changed from customer to staff - create employee record
                    if ($user->customer) {
                    ...
                    if (!$user->employee) {
                        Employee::create([
                            'user_id' => $user->id,
                            'employee_id' => $request->employee_id,
                            'job_title' => ucfirst($request->role),
                            'department' => $request->department ?? 'General',
            ...
            if ($user->employee) {
                ...
                if ($request->filled('hired_at')) {
                    $employeeData['hire_date'] = $request->hired_at;
                }
            ...
            return response()->json([
                ...
                'data' => $user->fresh(['customer', 'employee.manager.user'])
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy(Request $request, $id)
    {
        ...
        try {
            ...
            // Soft delete employee record if exists
            if ($user->employee) {
                $user->employee->delete();
            }
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->status === 'suspended') {
                $user->tokens()->delete();
                // Also suspend employee record
                if ($user->employee) {
                    $user->employee->update(['status' => 'suspended']);
                }
            } elseif ($request->status === 'active') {
                // Reactivate employee record if exists
            ...
            DB::commit();
            return response()->json(['message' => 'Status updated.', 'data' => $user->fresh(['employee'])]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    /**
     * Get staff users without employee records
     */
