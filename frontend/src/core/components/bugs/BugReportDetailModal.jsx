...
import {
  ...
  AlertCircle, FileText, ShieldAlert, Camera, ZoomIn,
} from 'lucide-react';
...
} from '../../../_shared/api/bugReportsAPI';
      ...
      {history.map((h, i) => (
        <div key={i} className="bug-flex bug-gap-3">
          <div className="bug-flex-col bug-items-center">
            <div className={`bug-timeline-dot ${i === 0 ? 'bug-timeline-dot-active' : 'bug-timeline-dot-inactive'}`} />
          ...
          <div className="bug-pb-4 bug-min-w-0">
            ...
            {h.note && <p className="bug-text-xs bug-text-secondary bug-mt-1">{h.note}</p>}
      ...
      >
        <div className="bug-modal">
          ...
          <div className="bug-flex bug-items-center bug-justify-between bug-px-6 bug-py-4 bug-border-b-light" style={{ flexShrink: 0 }}>
            ...
            <button onClick={onClose} className="bug-btn" style={{ padding: 6 }}>
              <X size={16} />
            </button>
          </div>
            ...
            {report && !loading && (
              <>
                ...
                <Section title="Report" icon={FileText}>
                  ...
                  {report.page_url && (
                    <a href={report.page_url} target="_blank" rel="noreferrer" className="bug-link" style={{ marginTop: 4 }}>
                      <ExternalLink size={11} /> Affected page
                    </a>
                  )}
                  {/* screenshot thumbnail — replaces old link */}
                ...
                <Section title="Details" icon={Hash}>
                  <div className="bug-grid-meta">
                    ...
                    <div>
                      <p className="bug-text-xs bug-text-muted" style={{ marginBottom: 2 }}>Tracking Token</p>
                      <span className="bug-mono bug-text-xs bug-text-secondary" style={{ wordBreak: 'break-all' }}>
                ...
                <Section title="Update Status" icon={Clock}>
                  <div className="bug-flex-col bug-gap-3">
                    <div className="bug-flex bug-items-center bug-gap-3">
                      <select value={newStatus} onChange={e => setNewStatus(e.target.value)} className="bug-select">
                        {BUG_STATUSES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                      </select>
                      <select value={newPriority} onChange={e => handlePrioritySave(e.target.value)} className="bug-select">
