...
const currencyAPI = {
  ...
  getBaseCurrency: async () => {
    const response = await api.get('/admin/currencies/base');
    return response.data;
  },

  // Set base currency 
  setBaseCurrency: async (currencyId) => {  // Changed: use ID, not code
    const response = await api.post('/admin/currencies/base', { 
  ...
  updateCurrency: async (currencyId, data) => {
    const response = await api.put(`/admin/currencies/${currencyId}`, data);
    return response.data;
  },

  // Toggle status 
  toggleStatus: async (currencyId, isActive) => {
    const response = await api.patch(`/admin/currencies/${currencyId}/status`, { 
      is_active: isActive 
    });
    return response.data;
  },
  ...
  updateAnchorRate: async (currencyId, anchorRate) => {
    const response = await api.patch(`/admin/currencies/${currencyId}/anchor-rate`, {
  ...
  createCurrency: async (currencyData) => {
    const response = await api.post('/admin/currencies', currencyData);
    return response.data;
  },
