        ...
        <div style={{
          ...
          <div style={{ padding: 8, borderBottom: '1px solid var(--color-border-tertiary)', position: 'relative' }}>
            ...
            <input
              ...
              placeholder="Search…"
              style={{ ...inputStyle, paddingLeft: 30, padding: '6px 8px 6px 28px', fontSize: '0.78rem' }}
            />
          </div>
...
function CoverImageUpload({ preview, onFileChange, onClear }) {
  ...
  return (
    <div>
      ...
      {preview ? (
        <div style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid var(--color-border-tertiary)' }}>
        ...
        <div
          ...
          style={{
            border: '2px dashed #dfbeff', borderRadius: 10,
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      const payload = {
        ...
        max_purchases_per_customer: form.max_purchases_per_customer ? Number(form.max_purchases_per_customer) : undefined,
        total_stock:                form.total_stock ? Number(form.total_stock) : undefined,
        eligibility_type:           form.eligibility_type,
        eligible_tiers:             form.eligibility_type === 'tier' ? selectedTiers : undefined,
        eligible_customer_types:    form.eligibility_type === 'customer_type' ? selectedTypes : undefined,
        valid_from:                 form.valid_from || undefined,
        valid_until:                form.valid_until || undefined,
      };

      const res = await hampersAPI.createHamper(payload);
      ...
      if (coverFile && newId) {
        ...
        await hampersAPI.uploadCoverImage(newId, fd).catch(() => {});
      }

      toast.success('Hamper created');
      navigate(`/admin/hampers/${newId}`);
    } catch (err) {
      ...
      if (apiErrors) {
        ...
        toast.error(err?.response?.data?.message || 'Failed to create hamper');
      }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '24px 0' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button type="button" onClick={() => navigate('/admin/hampers')} style={{ display: 'inline-flex', alignItems: 'center', fontSize: '0.82rem', fontWeight: 600, color: 'var(--color-text-secondary)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>
                <ChevronLeft size={15} />
              </button>
              <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  {form.eligibility_type === 'tier' && (
                    <MultiSelect
                      ...
                      hint="Customers must be in one of these tiers"
                      options={tiers}
                      selected={selectedTiers}
                      onChange={setSelectedTiers}
                      placeholder="Search and select tiers…"
                    />
                  )}
                  ...
                  {form.eligibility_type === 'customer_type' && (
                    <MultiSelect
                      ...
                      hint="Customers must have one of these types"
              ...
              <div style={card}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <Field label="Valid From" hint="Leave empty to start immediately">
                    <Input name="valid_from" type="datetime-local" value={form.valid_from} onChange={handleChange} />
                  </Field>
                  <Field label="Valid Until" hint="Leave empty for no end date">
                    <Input name="valid_until" type="datetime-local" value={form.valid_until} onChange={handleChange} />
                  </Field>
                </div>
              </div>
            </div>
            ...
            <div style={{ position: 'sticky', top: 96, display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={card}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <input
                    type="color" name="accent_color" value={form.accent_color}
