...
import {
  ...
  Clock, ChevronLeft, ChevronRight, Eye, EyeOff, Copy,
} from 'lucide-react';
import '../../styles/bug.css';
import GeneralLayout from '../../components/layout/GeneralLayout';
import {
  adminGetActiveKey, adminRegenerateKey, adminGetKeyLogs,
} from '../../api/bugReportsAPI';

function KeyDisplay({ keyData, onRegenerate, regenerating }) {
...
function LogsTable({ logs, meta, page, loading, onPage }) {
  return (
    <div className="bug-card">
      <div className="bug-flex bug-items-center bug-gap-2 bug-px-5 bug-py-4 bug-border-b-light">
        ...
        {meta && (
          <span className="bug-text-xs bug-text-muted" style={{ marginLeft: 'auto' }}>{meta.total} total</span>
        )}
      </div>
      ...
      {!loading && logs.map(log => (
        <div
          key={log.id}
          className="bug-grid-logs"
          style={{ borderBottom: '1px solid var(--bug-border-light)', fontSize: 14, padding: '14px 20px' }}
        >
          {/* result */}
          ...
          <div className="bug-flex-col bug-min-w-0" style={{ gap: 2 }}>
            ...
            <span className="bug-text-xs bug-text-muted truncate">{log.user_agent ?? '—'}</span>
          </div>

          {/* attempted key */}
          <span className="bug-mono bug-text-xs bug-text-muted truncate">{log.attempted_key}</span>
...
/**
 * AdminDevKeysPage — /admin/dev-keys
 */
