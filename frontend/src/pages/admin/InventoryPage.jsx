...
function Pagination({ meta, onPage }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, padding: "12px 14px", borderTop: `1px solid ${C.border}`, fontFamily: mono, fontSize: 11 }}>
      <span style={{ color: "#555" }}>
        {meta.from}–{meta.to} of {meta.total}
      </span>
      <ActionBtn small outline color={C.cyan} onClick={() => onPage(meta.current_page - 1)} disabled={meta.current_page === 1}>‹ Prev</ActionBtn>
      <ActionBtn small outline color={C.cyan} onClick={() => onPage(meta.current_page + 1)} disabled={meta.current_page === meta.last_page}>Next ›</ActionBtn>
    </div>
  );
}
...
function useToast() {
  ...
  // useCallback keeps ToastContainer referentially stable — without this, defining
  // it as a plain arrow function inside the hook recreates a new component type on
  // every render, causing React to unmount/remount every toast and re-fire effects.
...
// ─── Issue / Loan modal ───────────────────────────────────────────────────────
function IssueModal({ instance, items, mode = "issue", onClose, onSuccess, toast }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (mode === "issue") payload.due_date = form.due_date || undefined;

      if (mode === "issue") await inventoryAPI.assignments.issue(payload);
      else if (mode === "loan") await inventoryAPI.assignments.loan(payload);
      else if (mode === "department") await inventoryAPI.assignments.department(payload);
      else if (mode === "group") await inventoryAPI.assignments.group(payload);
  ...
  return (
    <Modal title={`// ${mode} item`} onClose={onClose}>
      {!instance && (
        <Field label="Item">
          <select style={selectStyle} value={form.item_id} onChange={e => set("item_id", e.target.value)}>
      ...
      <Field label="Assignee Type">
        <select style={selectStyle} value={form.assignee_type} onChange={e => set("assignee_type", e.target.value)}>
          {["employee", "customer", "department", "group"].map(t => <option key={t} value={t}>{t}</option>)}
        </select>
      </Field>
      <Field label="Assignee Label / Name">
        <input style={inputStyle} value={form.assignee_label} onChange={e => set("assignee_label", e.target.value)} placeholder="e.g. John Doe" />
      </Field>
      <Field label="Assignee ID (optional)">
        <input style={inputStyle} value={form.assignee_id} onChange={e => set("assignee_id", e.target.value)} placeholder="Internal ID" />
      </Field>
      {mode === "loan" && (
...
// ─── Repair modal ─────────────────────────────────────────────────────────────
function RepairModal({ instance, items, onClose, onSuccess, toast }) {
  const [form, setForm] = useState({
    instance_id: instance?.id ?? "",
    item_id: instance?.item_id ?? "",
    fault_description: "",
...
// ─── Repair Action modals (Send / Complete / Unrepairable) ────────────────────
...
// ─── New Instance modal ───────────────────────────────────────────────────────
function NewInstanceModal({ items, locations, onClose, onSuccess, toast }) {
  const [form, setForm] = useState({
    item_id: "", asset_tag: "", serial_number: "",
  ...
  return (
    <Modal title="// create return audit" onClose={onClose}>
      ...
      <Field label="Assignee Label">
        <input style={inputStyle} value={form.assignee_label} onChange={e => set("assignee_label", e.target.value)} placeholder="Name or label" />
      </Field>
      <Field label="Assignee ID (optional)">
        <input style={inputStyle} value={form.assignee_id} onChange={e => set("assignee_id", e.target.value)} />
      </Field>
  ...
  return (
    <Modal title="// add member" onClose={onClose}>
      <Field label="Member Type">
        <select style={selectStyle} value={form.member_type} onChange={e => set("member_type", e.target.value)}>
          {["employee", "customer", "department"].map(t => <option key={t} value={t}>{t}</option>)}
        </select>
      </Field>
      <Field label="Member Label / Name">
        <input style={inputStyle} value={form.member_label} onChange={e => set("member_label", e.target.value)} placeholder="Name" />
      </Field>
      <Field label="Member ID (optional)">
        <input style={inputStyle} value={form.member_id} onChange={e => set("member_id", e.target.value)} />
      </Field>
      <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
...
// ═══════════════════════════════════════════════════════════════════════════════
// SETTINGS MODAL (Categories + Locations)
// ═══════════════════════════════════════════════════════════════════════════════
...
function CatalogueTab({ onItemClick, toast }) {
  ...
  const load = useCallback(async (p = 1, s = search) => {
    setLoading(true);
    try {
      const res = await inventoryAPI.items.index({ page: p, per_page: 25, search: s || undefined });
  ...
  const handleSearch = (v) => {
    ...
    debounceRef.current = setTimeout(() => { setPage(1); load(1, v); }, 400);
  };
  ...
  return (
    <div>
        ...
        <Table
          ...
          cols={[
            { key: "name", label: "Item Name", render: (v, row) => (
              <span style={{ color: C.cyan, cursor: "pointer", fontWeight: 600 }} onClick={() => onItemClick(row.id)}>{v}</span>
            )},
            { key: "category", label: "Category", render: (_, row) => row.category?.name ?? "—" },
            { key: "type", label: "Type", render: v => <Chip label={v} /> },
            { key: "brand", label: "Brand", render: v => v || <span style={{ color: "#555" }}>—</span> },
            { key: "is_serialized", label: "Serialized", render: v => <span style={{ color: v ? C.green : "#555" }}>{v ? "Yes" : "No"}</span> },
      ...
      {modal === "new-item" && <NewItemModal categories={categories} locations={locations} onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
    </div>
  );
}
...
function InstancesTab({ onInstanceClick, toast }) {
  ...
  const load = useCallback(async (p = 1, s = search, sf = statusFilter) => {
    ...
    try {
      ...
      const res = await inventoryAPI.instances.index(params);
  ...
  const handleSearch = (v) => {
    setSearch(v);
    clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => { setPage(1); load(1, v, statusFilter); }, 400);
  };

  const handleStatus = (v) => { setStatusFilter(v); setPage(1); load(1, search, v); };
  const handlePage = (p) => { setPage(p); load(p); };
    ...
    <div>
      ...
      <Panel style={{ padding: 0 }}>
        <Table
          loading={loading}
          cols={[
            { key: "asset_tag", label: "Asset Tag", render: (v, row) => (
              <span style={{ color: C.cyan, fontWeight: 700, cursor: "pointer" }} onClick={() => onInstanceClick(row.id)}>{v}</span>
            )},
            ...
            { key: "location", label: "Location", render: (_, row) => row.location?.name ?? "—" },
            { key: "_view", label: "", render: (_, row) => (
              <ActionBtn color={C.cyan} outline small onClick={() => onInstanceClick(row.id)}>View ›</ActionBtn>
            )},
          ]}
          rows={instances}
        />
      ...
      {modal === "new-instance" && <NewInstanceModal items={items} locations={locations} onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
    </div>
  );
}
...
function AssignmentsTab({ toast }) {
  ...
  const load = useCallback(async (p = 1, s = search, f = filter) => {
    ...
    try {
      ...
      const res = await inventoryAPI.assignments.index(params);
  ...
  const handleSearch = (v) => {
    setSearch(v);
    clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => { setPage(1); load(1, v, filter); }, 400);
  };

  const handleFilter = (v) => { setFilter(v); setPage(1); load(1, search, v); };
  const handlePage = (p) => { setPage(p); load(p); };

  const markOverdue = async () => {
    try {
      await inventoryAPI.assignments.markOverdue();
      toast("Overdue sweep complete", "success");
      load(page);
  ...
  return (
    <div>
      ...
      <Panel style={{ padding: 0 }}>
        <Table
          ...
          cols={[
            ...
            { key: "status", label: "Status", render: v => <Chip label={v} /> },
            { key: "_actions", label: "", render: (_, row) => (row.status === "active" || row.status === "overdue") ? (
              <ActionBtn color={C.green} outline small onClick={() => setModal({ type: "return", assignment: row })}>Return</ActionBtn>
            ) : null },
          ]}
          rows={assignments}
        />
      ...
      {modal === "issue" && <IssueModal mode="issue" onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
      {modal === "loan" && <IssueModal mode="loan" onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
      {modal === "department" && <IssueModal mode="department" onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
      {modal?.type === "return" && <ReturnModal assignment={modal.assignment} onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
    </div>
  );
}
...
function RepairsTab({ toast }) {
  ...
  const load = useCallback(async (p = 1, f = filter) => {
    ...
    try {
      const params = { page: p, per_page: 25 };
      if (f !== "all") params.status = f;
      const res = await inventoryAPI.repairs.index(params);
      setRepairs(res?.data ?? res ?? []);
      setMeta(res?.meta ?? null);
  ...
  const handleFilter = (v) => { setFilter(v); setPage(1); load(1, v); };
  const handlePage = (p) => { setPage(p); load(p); };
  ...
  return (
    <div>
      ...
      {modal === "report" && <RepairModal items={items} onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
      {modal?.type === "send" && <RepairActionModal repair={modal.repair} action="send" onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
      {modal?.type === "complete" && <RepairActionModal repair={modal.repair} action="complete" onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
      {modal?.type === "unrepairable" && <RepairActionModal repair={modal.repair} action="unrepairable" onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
    </div>
  );
}
...
function DisputesTab({ toast }) {
  ...
  const load = useCallback(async (p = 1, f = filter) => {
    ...
    try {
      const params = { page: p, per_page: 25 };
      if (f !== "all") params.status = f;
      const res = await inventoryAPI.disputes.index(params);
  ...
  return (
    <div>
      ...
      <Panel style={{ padding: 0 }}>
        <Table
          ...
          cols={[
            ...
            { key: "_actions", label: "", render: (_, row) => row.status === "open" ? (
              <ActionBtn color={C.purple} outline small onClick={() => setModal({ type: "rule", dispute: row })}>Rule</ActionBtn>
            ) : null },
          ]}
          rows={disputes}
        />
      ...
      {modal === "open" && <DisputeModal assignments={assignments} onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
      {modal?.type === "rule" && <RuleModal dispute={modal.dispute} onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
    </div>
  );
}
...
function AuditsTab({ toast }) {
  ...
  const load = useCallback(async (p = 1, f = filter) => {
    ...
    try {
      const params = { page: p, per_page: 25 };
      if (f !== "all") params.status = f;
      const res = await inventoryAPI.audits.index(params);
  ...
  const finalise = async (audit) => {
    try {
      await inventoryAPI.audits.finalise(audit.id, { auto_process_returns: true });
      toast("Audit finalised", "success");
      load(page);
  ...
  return (
    <div>
      ...
      <Panel style={{ padding: 0 }}>
        <Table
          ...
          cols={[
            ...
            { key: "_actions", label: "", render: (_, row) => row.status !== "finalised" ? (
              <ActionBtn color={C.green} outline small onClick={() => finalise(row)}>Finalise</ActionBtn>
            ) : null },
          ]}
          rows={audits}
        />
      ...
      {modal === "create" && <AuditCreateModal onClose={() => setModal(null)} onSuccess={() => load(page)} toast={toast} />}
    </div>
  );
}
...
function GroupsTab({ toast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await inventoryAPI.groups.index();
      ...
      {loading ? (
        ...
        <Panel key={g.id} style={{ marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: expanded === g.id ? 16 : 0 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <ActionBtn small outline color={C.cyan} onClick={() => setExpanded(expanded === g.id ? null : g.id)}>
...
function LedgerTab({ toast }) {
  ...
  const load = useCallback(async (p = 1, tf = typeFilter) => {
    setLoading(true);
    try {
      const params = { page: p, per_page: 50 };
      if (tf !== "all") params.movement_type = tf;
      const res = await inventoryAPI.ledger.index(params);
  ...
  const handleTypeFilter = (v) => { setTypeFilter(v); setPage(1); load(1, v); };
  const handlePage = (p) => { setPage(p); load(p); };
...
function ExportTab({ toast }) {
  ...
  const runExport = async () => {
    setRunning(true);
    try {
      await inventoryAPI.export.run({ export_type: exportType, columns_included: selectedCols });
      toast("Export queued successfully", "success");
      loadLogs();
  ...
  const savePreset = async () => {
    ...
    try {
      await inventoryAPI.export.savePreset({ name: presetName, export_type: exportType, column_config: selectedCols });
  ...
  const loadPreset = (p) => {
    setExportType(p.export_type);
    setSelectedCols(p.column_config ?? []);
  };
...
export default function InventoryPage() {
  ...
  const [subPage, setSubPage] = useState(null); // { type: "item"|"instance", id }
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{
            ...
            border: `1px solid ${C.cyan}`, display: "flex", alignItems: "center", justifyContent: "center",
