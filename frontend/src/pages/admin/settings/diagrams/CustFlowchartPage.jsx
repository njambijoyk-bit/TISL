...
import custFlowchartHtml from './custflowchart.html?url';

export default function CustFlowchartPage() {
  const navigate = useNavigate();
  const [showNav, setShowNav] = useState(false);

  useEffect(() => {
    document.title = 'TISL Customer Flowchart — System Overview';
  }, []);

  // mouse hits left edge on desktop
  useEffect(() => {
  ...
  // touch swipe from left edge on mobile
  useEffect(() => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <span style={{ color: '#cdd6e0' }}>Customer Flowchart</span>
      </div>
      ...
      {/* iframe — pointer events blocked when nav open */}
      <iframe
        src={custFlowchartHtml}
        style={{
          width: '100%', height: '100%', border: 'none', display: 'block',
          pointerEvents: showNav ? 'none' : 'auto',
        }}
        title="TISL Customer Flowchart"
      />
