...
const AssignModal = ({ onClose, onAssign, currentAssignedId = null }) => {
  ...
  const fetchAdmins = async () => {
    ...
    try {
      const r = await api.get('/admin/users', { params: { role: 'admin', per_page: 100 } });
      setAdmins(r.data.data || r.data);
    } catch {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title={<span style={{ color: '#a855f7' }}>Assign Quote Request</span>} size="md">
      <form onSubmit={handleSubmit}>
        <p style={{ fontSize: '0.83rem', color: '#6b7280', marginBottom: 20, lineHeight: 1.65 }}>
          Select an admin to assign this quote request to. They will be responsible for reviewing and processing it.
        </p>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {loading ? (
            ...
            <p style={{ fontSize: '0.83rem', color: '#9ca3af', textAlign: 'center', padding: '16px 0' }}>No admin users found</p>
          ) : (
            <select
              value={selectedAdmin}
          ...
          <p style={{ fontSize: '0.7rem', color: '#9ca3af', marginTop: 6 }}>The selected admin will receive a notification about this assignment</p>
        </div>
        ...
        <div style={{ padding: '14px 16px', borderRadius: 12, background: purpleLt, border: `1px solid ${purpleBd}`, marginBottom: 24 }}>
          ...
          {[
            ...
            'The assigned admin will be notified',
