  ...
  return (
    <input
      ...
      style={{
        ...
        border: `1.5px solid ${focused ? purple : 'var(--border,#e5e7eb)'}`,
        boxShadow: focused ? `0 0 0 3px ${purpleLt}` : 'none',
...
const ItemCard = ({ children }) => (
  <div style={{
    borderRadius: 12, border: '1.5px solid var(--border,#f3f4f6)',
    overflow: 'hidden', background: 'var(--panel-bg,white)',
  }}>
    {children}
  </div>
);

const ItemCardHeader = ({ name, subname, badge, isCustom, imageUrl, onRemove, onPreview }) => (
  <div style={{
    padding: '12px 14px', background: 'var(--row-bg,rgba(249,250,251,0.7))',
    borderBottom: '1px solid var(--border,#f3f4f6)',
    display: 'flex', alignItems: 'flex-start', gap: 10,
  }}>
    {imageUrl && (
      <div
        onClick={e => { e.stopPropagation(); onPreview(imageUrl); }}
        style={{ width: 36, height: 36, borderRadius: 8, overflow: 'hidden', flexShrink: 0, cursor: 'zoom-in', background: '#f3f4f6' }}
      >
        <img
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      <p style={{ fontSize: '0.85rem', fontWeight: 800, color: purple, margin: '0 0 2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {name}
      </p>
      {subname && <p style={{ fontSize: '0.72rem', color: '#9ca3af', margin: 0 }}>{subname}</p>}
      {isCustom && (
        <span style={{ display: 'inline-block', fontSize: '0.65rem', fontWeight: 800, padding: '1px 7px', borderRadius: 9999, background: purpleLt, color: purple, marginTop: 3 }}>
          Custom
        </span>
      )}
      {badge && !isCustom && (
        <span style={{ display: 'inline-block', fontSize: '0.65rem', fontWeight: 700, padding: '1px 7px', borderRadius: 9999, background: '#f3f4f6', color: '#6b7280', marginTop: 3 }}>
          {badge}
        </span>
      )}
    </div>
    <button
      onClick={onRemove}
      style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: '#ef4444', display: 'flex', borderRadius: 6, flexShrink: 0 }}
    >
      <Trash2 size={15} />
    </button>
  </div>
...
const TabBtn = ({ tabKey, activeTab, onSelect, icon: Icon, label, count }) => {
...
const Step2SelectItems = ({ formData, updateFormData }) => {
  ...
  const toNumberOrNull = (v) =>
    v === '' || v === null || v === undefined ? null : (parseFloat(v) || null);
  ...
  const getServicePriceSuffix = (service) => {
    ...
    switch (service.pricing_model) {
      ...
      case 'project_based': return '/project';
      default:              return '';
    }
  };
  ...
  const handleServicesSelected = (services) => {
    updateFormData({
      selectedServices: [
        ...
        ...services.map(s => ({
          service_id: s.id, service: s, quantity: 1, estimated_hours: null,
          notes: '', is_custom: false, specifications: '',
          unit_of_measure: s.unit_of_measure ?? 'hour',
      ...
      updateFormData({
        selectedProducts: [
          ...formData.selectedProducts,
          {
            ...
            custom_details: {
              description: item.name, specifications: item.pricing_notes,
              budget: item.price, unit_of_measure: item.unit_of_measure ?? null,
      ...
      updateFormData({
        selectedServices: [
          ...formData.selectedServices,
          {
            ...
            estimated_hours: item.estimated_hours, notes: item.notes || '',
            is_custom: true, budget_per_unit: item.price ?? null,
            custom_details: {
              description: item.name, specifications: item.pricing_notes,
              budget: item.price, unit_of_measure: item.unit_of_measure ?? null,
              lead_time: item.lead_time ?? null,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* Header */}
      <div>
        <h2 style={{ fontSize: '1.3rem', fontWeight: 900, letterSpacing: '-0.02em', color: purple, margin: '0 0 5px' }}>
          Select Items
        </h2>
        <p style={{ fontSize: '0.83rem', color: '#9ca3af', margin: 0 }}>
          Choose products and services from our catalog or add custom items
        </p>
      </div>

      {/* Tabs */}
      <div style={{ borderBottom: '2px solid var(--border,#f3f4f6)', display: 'flex', gap: 2 }}>
        <TabBtn tabKey="products" activeTab={activeTab} onSelect={setActiveTab} icon={Package} label="Products" count={formData.selectedProducts.length} />
        <TabBtn tabKey="services" activeTab={activeTab} onSelect={setActiveTab} icon={Wrench}  label="Services" count={formData.selectedServices.length} />
      </div>
      ...
      {activeTab === 'products' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <Btn variant="primary" icon={<Package size={14} />} onClick={() => setShowProductSelector(true)}>
              Browse Products
            </Btn>
            <Btn variant="ghost" icon={<Plus size={14} />} onClick={() => { setEditingItem({ type: 'product' }); setShowCustomModal(true); }}>
              Add Custom Product
            </Btn>
          </div>
          ...
          {formData.selectedProducts.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <p style={{ fontSize: '0.75rem', fontWeight: 700, color: purple, margin: 0 }}>
                  ...
                  <ItemCardHeader
                    ...
                    subname={
                      !item.is_custom && item.product?.sku
                        ...
                        : (!item.is_custom && item.product?.price ? `Catalog: ${fmt(item.product.price)}` : null)
                    }
                  ...
                  <ItemCardBody>
                    <div className="s2-grid2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      ...
                      <div>
                        <FieldLabel>
                          Budget per Unit
                          {(item.is_custom ? item.custom_details?.unit_of_measure : item.unit_of_measure)
                            ? ` (${item.is_custom ? item.custom_details?.unit_of_measure : item.unit_of_measure})`
                            : ''}
                        </FieldLabel>
                        <StyledInput
                    ...
                    <div>
                      ...
                      <StyledTextarea
                        placeholder="Specs, model, size, color…"
      ...
      {activeTab === 'services' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <Btn variant="primary" icon={<Wrench size={14} />} onClick={() => setShowServiceSelector(true)}>
              Browse Services
            </Btn>
            <Btn variant="ghost" icon={<Plus size={14} />} onClick={() => { setEditingItem({ type: 'service' }); setShowCustomModal(true); }}>
              Add Custom Service
            </Btn>
          </div>
              ...
              {formData.selectedServices.map((item, index) => (
                <ItemCard key={index}>
                  <ItemCardHeader
                    ...
                    subname={
                      !item.is_custom && item.service?.short_description
                        ? item.service.short_description
                        : (() => {
                            const cp = getServiceUnitPrice(item.service);
                            return cp ? `Catalog: ${fmt(cp)}${getServicePriceSuffix(item.service)}` : null;
                          })()
                    }
                    isCustom={item.is_custom}
                  ...
                  <ItemCardBody>
                    ...
                    <div>
                      <FieldLabel>Specifications</FieldLabel>
                      <StyledTextarea
                        placeholder="Scope, requirements, tools, location constraints…"
                        value={item.is_custom ? (item.custom_details?.specifications || '') : (item.specifications || '')}
                        onChange={e => item.is_custom
      ...
      {showCustomModal && (
        <CustomItemModal
          ...
          onClose={() => { setShowCustomModal(false); setEditingItem(null); }}
      ...
      {previewImage && (
        <div
          style={{ position: 'fixed', inset: 0, zIndex: 70, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          onClick={() => setPreviewImage(null)}
        >
          <button
            onClick={() => setPreviewImage(null)}
            style={{ position: 'absolute', top: 16, right: 16, background: 'rgba(0,0,0,0.5)', border: 'none', borderRadius: '50%', padding: 8, cursor: 'pointer', display: 'flex' }}
          >
